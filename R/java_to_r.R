...
#'
#' @param java A character string containing the Java code to be converted to R,
#' or a filename with the Java code.
#' @param ... Additional arguments to pass to the chat_completion() function.
#'
#' @return If the input is a character string, the resulting R code will be
#' printed and returned as a character string. If the input is a filename, the
#' resulting R code will be saved as a file with the same name as the input
#' file, but with the extension `.R`, and the filename will be returned.
#'
#' @details This function is not guaranteed to provide perfect conversions and
#' might produce invalid R code in some cases. Users are encouraged to verify
#' the output code carefully.
#'
#' @examples
#' \dontrun{
...
java_to_r <- function(java, ...) {
  ...
  if (is_r(output) == FALSE) {
    cli::cli_alert_warning(paste0("The conversion from Java to R has ",
