...
#'
#' This function takes an R script containing traditional (nested syntax) R
#' code and converts it to magrittr-style syntax, using the pipe (%>%) operator.
#' It also validates the input and output code to ensure proper R syntax.
#'
#' @param r A file path or character string containing the R code to be
#' converted.
#' @param n_tokens_limit The maximum number of tokens allowed in the input text
#' (default: 3000).
#' @param ... Additional arguments to pass to the chat_completion() function.
#'
#' @return If r is a character string, the function returns the converted R code
#' as a character string. If r is a file path, the function writes the converted
#' code to a new file with the same name and a "-pipe.R" suffix, and returns the
...
nested_to_pipe <- function(r, n_tokens_limit=3000, ...) {
  ...
  if (is_r(output) == FALSE) {
    cli::cli_alert_warning(paste0("The conversion has potentially resulted in ",
      "invalid R code. Please verify the output code carefully!"))
  }
  ...
  if (filename == "character string") {
    ...
    output_file <- paste0(replace_file_extension(filename, new_extension = ""),
      "-pipe.R")
