...
#'
#' @param filename character string indicating the name of the text file to
#' read in
#' @param batch_size integer indicating the number of lines to read in per batch
#' (default is 1000)
#' @param encoding character string indicating the encoding to use
#' (default is "cl100k_base")
#' @return a numeric value indicating the total number of tokens in the text
#' file
#'
num_tokens_file <- function(filename, batch_size = 1000, 
  ...
  all_token_counts <- readr::read_lines_chunked(filename,
    callback = readr::ListCallback$new(tokens_batch), chunk_size = batch_size)
